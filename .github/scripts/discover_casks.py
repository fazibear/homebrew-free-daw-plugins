...
PLUGIN_NAMES = {
    "elarmuzik1993/distortion": ("sledge-distortion", "Sledge Distortion"),
}
...
def main(source):
    ...
    for repo in repositories.values():
        ...
        if not asset:
            continue
        name, display = PLUGIN_NAMES.get(repo["full_name"].lower(), (slug(repo["name"]), re.sub(r"[-_]+", " ", repo["name"]).strip().title()))
