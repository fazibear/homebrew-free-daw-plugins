...
def plugins4free_results():
    directory = "https://plugins4free.com/instruments"
    request = urllib.request.Request(directory, headers={"User-Agent": "Mozilla/5.0 (compatible; cask-discovery/1.0)"})
    with urllib.request.urlopen(request, timeout=30) as response:
        html = response.read(2_000_000).decode("utf-8", "ignore")
    results = []
    for match in re.finditer(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>(.*?)</a>', html, re.I | re.S):
        result_url = urllib.parse.urljoin(directory, match.group(1))
        title = re.sub(r"<[^>]+>", "", match.group(2)).strip()
        if result_url.startswith("https://plugins4free.com/") and title:
...
def clean_plugin_name(repository_name):
    cleaned = re.sub(r"(?:[-_ ]+(?:vst3?|au|clap|lv2|plugin|plugins|audio))+$", "", repository_name, flags=re.I)
    return cleaned.strip("-_ ") or repository_name
...
def web_installer(result):
    ...
    for link in links:
        ...
        is_macos = re.search(r"(mac|macos|darwin|osx|universal|au2?|vst3|clap)", absolute, re.I)
        is_dmg = re.search(r"\.dmg(\?|$)", absolute, re.I)
        is_installer = re.search(r"\.(pkg|dmg)(\?|$)", absolute, re.I)
        if is_installer and (is_macos or is_dmg) and not re.search(r"(windows|win32|linux|ubuntu)\b", absolute, re.I):
...
def main(source):
    ...
    web_results = []
    if source == "web":
        web_results.extend(plugins4free_results())
    ...
    for repo in repositories.values():
        ...
        if not asset:
            continue
        cleaned_name = clean_plugin_name(repo["name"])
        name, display = slug(cleaned_name), re.sub(r"[-_]+", " ", cleaned_name).strip().title()
        name, content = render(name, display,
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument("--source", choices=("github", "web"), required=True)
